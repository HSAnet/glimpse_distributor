...
# Should-Start:      nginx
# Should-Stop:       nginx
# Short-Description: distributor django website initscript
# Description:       This will start the distributor django website in fastcgi mode.
### END INIT INFO


PATH="/sbin:/usr/sbin:/bin:/usr/bin";
DESC="distributor service";
NAME="distributor";
PROJECT_PATH="/var/www/distributor";
...
ACCESSLOG="$PROJECT_PATH/deploy/logs/access.log";
DAEMON="$PROJECT_PATH/bin/gunicorn";
DAEMON_ARGS="$NAME.wsgi:application --chdir $PROJECT_PATH --daemon --umask 0000 --bind unix:$SOCKETFILE --pid $PIDFILE --error-logfile $ERRLOG --access-logfile $ACCESSLOG"; # out log?
